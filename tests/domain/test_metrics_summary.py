...
from app.domain.metrics.summary import compute_flow_metrics
...
def test_compute_flow_metrics_aggregates_the_trailing_window() -> None:
    samples = [
    ...
    metrics = compute_flow_metrics(samples, now=NOW)
...
def test_compute_flow_metrics_includes_blocked_time_of_window_completions() -> None:
    metrics = compute_flow_metrics([_sample(20, 10, 0, blocked_days=5)], now=NOW)

    assert metrics.blocked_time == timedelta(days=5)
    assert metrics.flow_efficiency == 0.5


def test_compute_flow_metrics_with_no_samples_is_empty() -> None:
    metrics = compute_flow_metrics([], now=NOW)
